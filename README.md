...
**http://72.56.92.32/** — развёрнуто на сервере (Ubuntu, Docker: nginx + gunicorn + PostgreSQL).

| Раздел | Ссылка |
|---|---|
| Витрина | http://72.56.92.32/ |
| Swagger UI | http://72.56.92.32/api/docs/ |
| ReDoc | http://72.56.92.32/api/redoc/ |
| GraphQL (для staff) | http://72.56.92.32/graphql/ |
| Админка | http://72.56.92.32/admin/ |

Тестовый покупатель: `buyer@example.com` / `buyerpass123` (есть доставленный заказ).
Полный деплой описан в [`deploy/DEPLOY.md`](deploy/DEPLOY.md).
